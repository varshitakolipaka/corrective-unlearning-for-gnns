...
def get_script(dataset, unlearning_model, attack, seed, data_dir, cf=1.0, df_size=0.5, db_name=None, log_name='default', gnn='gcn', linked=False):    
    dataset_to_df = {
        'Amazon': 10000,
        'Cora': 750,
        'CS': 3000,
        ...
        for seed in range(args.start_seed, args.end_seed):
            for unlearning_model in unlearning_models:
                for attack in attacks:
                    for cf in cfs:
                        script = get_script(dataset, unlearning_model, attack, seed, args.data_dir, cf, df_size=args.df_size, db_name=args.db_name, log_name=args.log_name, gnn=args.gnn, linked=args.linked)
                        # print(script)
                        os.system(script)